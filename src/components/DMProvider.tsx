...
export function DMProvider({ children, config }: DMProviderProps) {
  ...
  >({
    mutationFn: async ({ recipientPubkey, content, attachments = [] }) => {
      ...
      const senderGiftWrapContent = await senderRandomSigner.nip44!.encrypt(
        user.pubkey,
        JSON.stringify(senderSeal)
      );
      ...
      try {
        ...
        if (recipientResult.status === 'rejected' && senderResult.status === 'rejected') {
          throw new Error(
            `Both gift wraps rejected. Recipient: ${recipientResult.reason}, Sender: ${senderResult.reason}`
          );
        }
      } catch (publishError) {
  ...
  const loadPastNIP4Messages = useCallback(
    async (sinceTimestamp?: number) => {
      ...
      setScanProgress((prev) => ({
        ...prev,
        nip4: { current: 0, status: SCAN_STATUS_MESSAGES.NIP4_STARTING },
      }));

      while (processedMessages < DM_CONSTANTS.SCAN_TOTAL_LIMIT) {
        const batchLimit = Math.min(
          DM_CONSTANTS.SCAN_BATCH_SIZE,
          DM_CONSTANTS.SCAN_TOTAL_LIMIT - processedMessages
        );
        ...
        try {
          const batchDMs = await nostr.query(filters, {
            signal: AbortSignal.timeout(DM_CONSTANTS.NIP4_QUERY_TIMEOUT),
          });
          ...
          const oldestToMe =
            validBatchDMs.filter((m) => m.pubkey !== user.pubkey).length > 0
              ? Math.min(
                  ...validBatchDMs.filter((m) => m.pubkey !== user.pubkey).map((m) => m.created_at)
                )
              : Infinity;
          const oldestFromMe =
            validBatchDMs.filter((m) => m.pubkey === user.pubkey).length > 0
  ...
  const loadPastNIP17Messages = useCallback(
    async (sinceTimestamp?: number) => {
      if (!user?.pubkey) return;
      ...
      setScanProgress((prev) => ({
        ...prev,
        nip17: { current: 0, status: SCAN_STATUS_MESSAGES.NIP17_STARTING },
      }));
      ...
      while (processedMessages < DM_CONSTANTS.SCAN_TOTAL_LIMIT) {
        ...
        try {
          const batchEvents = await nostr.query(filters, {
  ...
  const queryRelaysForMessagesSince = useCallback(
    async (
  ...
  const decryptNIP4Message = useCallback(
    async (event: NostrEvent, otherPubkey: string): Promise<DecryptionResult> => {
      try {
        if (user?.signer?.nip04) {
  ...
  const sortAndUpdateParticipantState = useCallback(
    (participant: {
      messages: DecryptedMessage[];
      lastActivity: number;
      lastMessage: DecryptedMessage | null;
  ...
  const addMessageToState = useCallback(
    (message: DecryptedMessage, conversationPartner: string, protocol: MessageProtocol) => {
      setMessages((prev) => {
        ...
        if (existing) {
          ...
          const optimisticIndex = existing.messages.findIndex(
            (msg) =>
              msg.isSending &&
  ...
  const processIncomingNIP4Message = useCallback(
  ...
  const processNIP17GiftWrap = useCallback(
    async (event: NostrEvent): Promise<NIP17ProcessingResult> => {
      if (!user?.signer?.nip44) {
        return {
          processedMessage: {
            ...event,
      ...
      try {
        ...
        return {
          processedMessage: {
            ...
            id:
              messageEvent.id ||
              `missing-nip17-inner-${messageEvent.created_at}-${messageEvent.pubkey.substring(0, 8)}-${messageEvent.content.substring(0, 16)}`,
        ...
        return {
          processedMessage: {
            ...
            decryptedContent: '',
            error:
              error instanceof Error ? error.message : 'Failed to decrypt or parse NIP-17 message',
          },
          conversationPartner: event.pubkey,
          sealEvent: event, // Return the gift wrap as fallback
        };
  ...
  const processIncomingNIP17Message = useCallback(
    async (event: NostrEvent) => {
      if (!user?.pubkey) return;
  ...
  const startNIP4Subscription = useCallback(
    async (sinceTimestamp?: number) => {
  ...
  const startNIP17Subscription = useCallback(
    async (sinceTimestamp?: number) => {
  ...
  const loadAllCachedMessages = useCallback(async (): Promise<{
    nip4Since?: number;
    nip17Since?: number;
  }> => {
    ...
    try {
      ...
      const filteredParticipants = enableNIP17
        ...
        : Object.fromEntries(
            Object.entries(cachedStore.participants).filter(
              ([_, participant]) => !participant.hasNIP17
            )
          );
      ...
      for (const [participantPubkey, participant] of Object.entries(filteredParticipants)) {
        const processedMessages = await Promise.all(
          participant.messages.map(async (msg) => {
            ...
            return {
              ...
              id:
                msg.id ||
                `missing-${msg.kind}-${msg.created_at}-${msg.pubkey.substring(0, 8)}-${msg.content?.substring(0, 16) || 'nocontent'}`,
              decryptedContent,
              error,
        ...
        newState.set(participantPubkey, {
          ...
          lastMessage:
            processedMessages.length > 0 ? processedMessages[processedMessages.length - 1] : null,
  ...
  useEffect(() => {
    const relayChanged =
      JSON.stringify(previousRelayMetadata.current) !== JSON.stringify(appConfig.relayMetadata);
    ...
    const handleHardRefresh = (e: KeyboardEvent) => {
      if ((e.ctrlKey || e.metaKey) && e.shiftKey && (e.key === 'R' || e.key === 'r')) {
        try {
          ...
          console.warn(
            "[DM] SessionStorage unavailable, cache won't clear on hard refresh:",
            error
          );
        }
      }
    ...
    try {
      ...
      messages.forEach((participant, participantPubkey) => {
        messageStore.participants[participantPubkey] = {
          messages: participant.messages.map(
            (msg) =>
              ({
  ...
  const sendMessage = useCallback(
    async (params: {
      ...
      addMessageToState(
        optimisticMessage,
        recipientPubkey,
        protocol === MESSAGE_PROTOCOL.NIP04 ? MESSAGE_PROTOCOL.NIP04 : MESSAGE_PROTOCOL.NIP17
      );
  ...
  const isDoingInitialLoad =
    isLoading && (loadingPhase === LOADING_PHASES.CACHE || loadingPhase === LOADING_PHASES.RELAYS);
  ...
  return <DMContext.Provider value={contextValue}>{children}</DMContext.Provider>;
}
