...
const LoginDialog: React.FC<LoginDialogProps> = ({ isOpen, onClose, onLogin, onSignup }) => {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent
        className={cn(
          'max-w-[95vw] sm:max-w-md max-h-[90vh] max-h-[90dvh] p-0 overflow-hidden rounded-2xl overflow-y-scroll'
        )}
      >
        <DialogHeader className={cn('px-6 pt-6 pb-1 relative')}>
          {/* Visually hidden title satisfies the Radix Dialog a11y requirement
              (screen readers) without changing the existing visual layout. */}
            ...
            <TabsContent value="bunker" className="space-y-3 bg-muted">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  autoComplete="off"
                />
                {errors.bunker && <p className="text-sm text-red-500">{errors.bunker}</p>}
              </div>
